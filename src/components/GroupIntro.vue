    ...
    <ul class="groups">
      <li class="group" v-for="(group, index) in groups" :key="index">
        <div class="group-info">
          ...
          <h4>{{ group.name_en }}</h4>
          <p
            class="group-info-desc"
            v-html="'<p>' + group.intro.replace(/\<br\>/g, '</p><p>') + '</p>'"
          />
        ...
        <div class="group-slide">
          <el-carousel v-if="group.slides.length > 1" height="280px">
            <el-carousel-item
              v-for="(slide, index) in group.slides"
              :key="index"
            >
              <div class="group-slide-item">
                <div
                  v-html="
                    '<p>' + slide.text.replace(/\<br\>/g, '</p><p>') + '</p>'
                  "
                />
                <img
                  v-if="slide.img"
          ...
          <div v-if="group.slides.length === 1">
            <div class="group-slide-item">
              <div
                v-html="
                  '<p>' +
                  group.slides[0].text.replace(/\<br\>/g, '</p><p>') +
                  '</p>'
                "
              />
              <img :src="require('../assets/image/' + group.slides[0].img)" />
            </div>
          </div>
        </div>
      </li>
